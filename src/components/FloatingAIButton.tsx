...
const FloatingAIButton = () => {
  ...
  const { isAuthenticated, isPremium, userBatchSubscriptions } = useAuth();
  ...
  // 2. Either has premium subscription OR has purchased at least one batch AND
  // 3. Not on test pages
  const hasAIAccess = isAuthenticated && (isPremium || (userBatchSubscriptions && userBatchSubscriptions.length > 0)) && !isTestPage;
  ...
  return (
    <>
      {/* Floating Button - Only show if user has AI access */}
      {hasAIAccess && (
        <button
            ...
            <div className="absolute bottom-full right-0 mb-2 animate-in fade-in slide-in-from-bottom-2 duration-200">
              <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-4 py-2 rounded-lg shadow-xl whitespace-nowrap">
                ...
                <p className="text-xs opacity-90">
                  JEEnie - Included with Batch Purchase ✨
                </p>
              </div>
              {/* Arrow */}
